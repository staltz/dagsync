...
class DAGSyncStream extends Pipeable {
  ...
  sendInitBloomReq(id, commonRange) {
    // prettier-ignore
    this.#debug('%s Stream IN: received common range %o for id %s', this.#myId, commonRange, id)
    this.#commonRange.set(id, commonRange)
  ...
  #sendInitBloomRes(id, remoteBloom) {
    ...
    // prettier-ignore
    this.#debug('%s Stream OUT: send bloom round 0 plus msgIDs for %s', this.#myId, id)
  }

  #sendBloomReq(id, phase, round, remoteBloom, msgIDsForMe) {
    // prettier-ignore
    this.#debug('%s Stream IN: received bloom round %s plus msgIDs for %s: %o', this.#myId, round-1, id, msgIDsForMe)
    const commonRange = this.#commonRange.get(id)
    ...
    // prettier-ignore
    this.#debug('%s Stream OUT: send bloom round %s plus msgIDs for %s: %o', this.#myId, round, id, msgIDsForThem)
  }

  #sendBloomRes(id, phase, round, remoteBloom, msgIDsForMe) {
    // prettier-ignore
    this.#debug('%s Stream IN: received bloom round %s plus msgIDs for %s: %o', this.#myId, round, id, msgIDsForMe)
    const commonRange = this.#commonRange.get(id)
    this.#updateReceivableMsgs(id, msgIDsForMe)
    const msgIDsForThem = this.#algo.msgsMissing(
      id,
  ...
  #sendLastBloomRes(id, phase, round, remoteBloom, msgIDsForMe) {
    ...
    // prettier-ignore
    this.#debug('%s Stream OUT: send msgIDs for %s: %o', this.#myId, id, msgIDsForThem)
  }

  #sendMissingMsgsReq(id, msgIDsForMe) {
    // prettier-ignore
    this.#debug('%s Stream IN: received msgIDs for %s: %o', this.#myId, id, msgIDsForMe)
    this.#updateReceivableMsgs(id, msgIDsForMe)
    ...
    this.#algo.getMsgs(msgIDs, (err, msgs) => {
      ...
      // prettier-ignore
      this.#debug('%s Stream OUT: send msgs for %s: %o', this.#myId, id, msgs)
  ...
  #sendMissingMsgsRes(id, msgsForMe) {
    // prettier-ignore
    this.#debug('%s Stream IN: received msgs for %s: %o', this.#myId, id, msgsForMe)
    const msgIDs = this.#sendableMsgs.has(id)
      ? [...this.#sendableMsgs.get(id)]
      : []
    this.#algo.getMsgs(msgIDs, (err, msgs) => {
